...
class RF_CMI_CLASS(object):

    def __init__(self,  hidden_size = 32, lr=0.001,inner_boost =2,maxiter=100):
    ...
    def get_cmi(self, x, y, z):
        ...
        nbrs = NearestNeighbors(n_neighbors=1).fit(z.cpu().data.numpy())
        indx = nbrs.kneighbors(z.cpu().data.numpy(), return_distance=False).flatten()
        yp = y[indx]

        

        y_joint = torch.cat([y, z],1)
        y_marginal = torch.cat([yp, z],1)

        self.data_joint_train_seq =     x[:train_partition]
        self.data_marginal_train_seq =  x[:train_partition]
        
        self.data_joint_train_vec =     y_joint[:train_partition]
        self.data_marginal_train_vec =  y_marginal[:train_partition]

        self.data_joint_eval_seq =      x[train_partition:]
        self.data_marginal_eval_seq =   x[train_partition:]       
        
        self.data_joint_eval_vec =      y_joint[train_partition:]
        self.data_marginal_eval_vec =   y_marginal[train_partition:]

        self.classifier = V2Cuda(Classifer(input_dim=( x.size()[1], y_joint.size()[1] ), hidden_size= self.hidden_size*2, embed_size=self.hidden_size ))
    ...
    def get_mi(self, x, y, x_shuffle_dim=None):
        ...
        train_partition = int(x.size()[0]//2)

        #data_joint = torch.cat([x,y],1)
        indx = np.random.permutation(x.size()[0])
        #data_marginal = torch.cat([x,y[indx]],1)
        if x_shuffle_dim is not None:
            xmarginal = torch.cat([x[:,:x_shuffle_dim],x[indx,x_shuffle_dim:]])
